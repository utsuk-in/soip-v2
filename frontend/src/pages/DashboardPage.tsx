...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      setLoading(false);
      // Load feedback for cached opportunities
      const allIds = [...new Set([...cached.recommended, ...cached.expiring].map((o) => o.id))];
      loadFeedback(allIds);
      return;
    }
    ...
    async function load() {
      try {
        ...
        // Load feedback for all visible opportunities
        const allIds = [...new Set([...recommendedTop, ...soon].map((o) => o.id))];
        loadFeedback(allIds);
      } finally {
        setLoading(false);
      }
