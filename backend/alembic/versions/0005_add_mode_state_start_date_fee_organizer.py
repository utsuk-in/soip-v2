...
# City-to-state mapping for backfill
CITY_TO_STATE = {
...
def upgrade() -> None:
    ...
    # 2. Add columns (mode nullable initially for backfill)
    op.add_column("opportunities", sa.Column("mode", sa.Enum("online", "offline", "hybrid", name="opportunitymode", create_type=False), nullable=True, server_default="online"))
    ...
    op.add_column("opportunities", sa.Column("fee_type", sa.Enum("free", "paid", name="feetype", create_type=False), nullable=True))
    op.add_column("opportunities", sa.Column("organizer", sa.String(300), nullable=True))

    # 3. Backfill mode from domain_tags and location
    conn = op.get_bind()

    # Default all to 'online' first
    conn.execute(sa.text("UPDATE opportunities SET mode = 'online' WHERE mode IS NULL"))

    # Hybrid: domain_tags has both online and offline
    conn.execute(sa.text("""
        UPDATE opportunities SET mode = 'hybrid'
        WHERE domain_tags::jsonb ? 'online' AND domain_tags::jsonb ? 'offline'
    """))

    # Offline: location is a physical place (not online/virtual/remote)
    conn.execute(sa.text("""
    ...
    # Upgrade offline to hybrid if domain_tags also has 'online'
    conn.execute(sa.text("""
        UPDATE opportunities SET mode = 'hybrid'
        WHERE mode = 'offline'
    ...
    # 4. Backfill state from location
    for city, state in CITY_TO_STATE.items():
        conn.execute(
            sa.text("UPDATE opportunities SET state = :state WHERE state IS NULL AND lower(location) LIKE :pattern"),
    ...
    # 5. Clean online/offline from domain_tags
    conn.execute(sa.text("""
    ...
    # Fix any empty arrays after cleanup
    conn.execute(sa.text("""
        UPDATE opportunities
